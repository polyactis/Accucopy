/*
 * 2013.09.22 YH: copied along with RedBlackTree.h as it uses functions from misc.h
 */
...
inline void * SafeMalloc(size_t size) {
  ...
  if ( (result = malloc(size)) ) { /* assignment intentional */
    ...
    sprintf(errorMessagePartOne,
	    "Exiting From SafeMalloc because malloc of size %zu failed.\n", size);
...
inline void * SafeCalloc(int numberOfElements , size_t size) {
  ...
  if ( (result = calloc(numberOfElements, size)) )
    ...
    printf("memory overflow: calloc failed in SafeCalloc(%i, %zu).",
